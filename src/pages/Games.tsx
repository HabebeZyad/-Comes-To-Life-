...
export default function Games() {
  ...
  if (currentGame !== 'menu') {
    ...
    if (GameComponent) {
      return (
        <Suspense fallback={<PageLoader className="h-[60vh]" />}>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {showLeaderboard && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="mb-12">
            <Suspense fallback={<div className="h-48 flex items-center justify-center rounded-lg border border-gold/20 bg-black/35 backdrop-blur-md"><Sparkles className="animate-pulse text-gold" /></div>}>
              <Leaderboard />
            </Suspense>
          </motion.div>
        )}
