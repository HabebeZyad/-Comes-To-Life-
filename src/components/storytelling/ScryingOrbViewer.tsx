...
const Viewer360 = ({ image }: { image: string }) => {
    ...
    const [zoomStage, setZoomStage] = useState(0); // 0: Normal, 1: Zoom, 2: Super
    const targetFov = useRef(80);
    const targetLookAt = useRef(new THREE.Vector3(1, 0, 0));
    const currentLookAt = useRef(new THREE.Vector3(1, 0, 0));
...
/**
 * ScryingOrbViewer Component
 * Heavy 3D component utilizing Three.js and OrbitControls.
 * Extracted to allow for lazy loading and isolate dependencies (~1MB).
 */
const ScryingOrbViewerComponent = ({ image }: { image: string }) => {
    return (
        <motion.div
            key="viewer"
            initial={{ opacity: 0 }}
...
export const ScryingOrbViewer = memo(ScryingOrbViewerComponent);
ScryingOrbViewer.displayName = 'ScryingOrbViewer';
