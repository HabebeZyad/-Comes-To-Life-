        ...
        <EgyptianCard variant="tomb" padding="lg">
              ...
              <div className="grid md:grid-cols-3 gap-6 max-w-3xl mx-auto">
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  ...
                  >
                    ...
                    <span className="text-sm opacity-80">Slow blocks • 90s</span>
                  </EgyptianButton>
                </motion.div>
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <EgyptianButton
                  ...
                  >
                    ...
                    <span className="text-sm opacity-80">Normal speed • 60s</span>
                  </EgyptianButton>
                </motion.div>
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <EgyptianButton
                  ...
                  >
                    ...
                    <span className="text-sm opacity-80">Fast blocks • 45s</span>
                  </EgyptianButton>
                </motion.div>
              </div>
            </div>
              ...
              >
                ...
                {blocks.map((block, index) => (
                  block.placed && (
                    ...
                    >
                      <div className="w-full h-full flex items-center justify-center text-sm font-display text-obsidian/70">
                        𓊪
                      </div>
                    </motion.div>
                  )
                ...
                {isPlaying && currentBlockIndex < blocks.length && (
                  ...
                  >
                    <div className="w-full h-full flex items-center justify-center text-sm font-display text-obsidian">
                      𓊪
                    </div>
                  </motion.div>
                )}
                ...
                {isPlaying && (
                  <div 
                    className="absolute border-2 border-dashed border-primary/40 rounded"
                    style={{
                      width: blocks[currentBlockIndex]?.width || 0,
                      height: blockHeight - 4,
                      left: (containerWidth - (blocks[currentBlockIndex]?.width || 0)) / 2,
                      bottom: currentBlockIndex * blockHeight,
                    }}
                  />
                )}
              </div>
              ...
              <div className="space-y-2 mb-8">
                ...
                <div className="flex items-center justify-center gap-1 mt-4">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={i < Math.min(5, Math.floor(perfectDrops / 2) + 1) ? 'text-primary fill-primary' : 'text-muted'}
                      size={32}
                    />
                  ))}
                </div>
          ...
          {gameLost && (
            ...
            >
              ...
              <h2 className="text-5xl font-display text-terracotta mb-4">
                {timeLeft === 0 ? "Time's Up!" : "Block Misaligned!"}
              </h2>
              <div className="space-y-2 mb-8">
