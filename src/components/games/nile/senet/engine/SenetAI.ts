...
export class SenetAI {
  ...
  private static evaluateState(state: GameState, player: Player): number {
    ...
    // 1. Progress: distance from start (higher is better)
    ...
    // 2. Pieces off board (high priority)
  ...
  private static expectimax(state: GameState, depth: number, isMaximizing: boolean, originalPlayer: Player): number {
    ...
    if (isMaximizing) {
      return this.averageOverThrows(state, depth, true, originalPlayer);
    } else {
      return this.averageOverThrows(state, depth, false, originalPlayer);
    }
  }
