        ...
        <EgyptianCard variant="tomb" padding="lg">
          ...
          {!isPlaying && !gameWon && !gameLost ? (
            <div className="text-center py-12">
              <div className="text-8xl mb-6">?«?ƒÌ??</div>
              ...
              <div className="grid md:grid-cols-3 gap-6 max-w-3xl mx-auto">
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <EgyptianButton variant="turquoise" size="xl" onClick={() => initializeGame('easy')} className="w-full h-auto flex-col py-8">
                    ...
                    <span className="text-sm opacity-80">Slow blocks Ÿ?  90s</span>
                  </EgyptianButton>
                </motion.div>
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <EgyptianButton variant="default" size="xl" onClick={() => initializeGame('medium')} className="w-full h-auto flex-col py-8">
                    ...
                    <span className="text-sm opacity-80">Normal speed Ÿ?  60s</span>
                  </EgyptianButton>
                </motion.div>
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <EgyptianButton variant="danger" size="xl" onClick={() => initializeGame('hard')} className="w-full h-auto flex-col py-8">
                    <div className="text-4xl mb-2">Ÿ√…</div>
                    <span className="text-xl">Hard</span>
                    <span className="text-sm opacity-80">Fast blocks Ÿ?  45s</span>
                  </EgyptianButton>
                </motion.div>
              </div>
            </div>
              ...
              >
                ...
                {blocks.map((block, index) => (
                  block.placed && (
                    ...
                    >
                      <div className="w-full h-full flex items-center justify-center text-sm font-display text-obsidian/70">?ÙË“</div>
                    </motion.div>
                  )
                ))}
                {isPlaying && currentBlockIndex < blocks.length && (
                  ...
                  >
                    <div className="w-full h-full flex items-center justify-center text-sm font-display text-obsidian">?ÙË“</div>
                  </motion.div>
                )}
                {isPlaying && (
                ...
                <AnimatePresence>
                  {showSparkle && (
                    ...
                    >
                      ...
                      <div className="absolute text-4xl">Ÿ£–</div>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
          ...
          {gameWon && (
            ...
            >
              <div className="text-9xl mb-6">?«?ƒÌ??</div>
          ...
          {gameLost && (
            ...
            >
              <div className="text-9xl mb-6">?«˙Õ</div>
